...
contract SinglePriceNFTcontract is ERC721Enumerable, Ownable {
    using Strings for uint256;
    uint256 public maxSupply = 10000;
    uint256 public currentSupply = 1000;
    uint256 public assetPrice = 0.08 * 10 ** 18;
    ...
    constructor(address _w1, address _w2, address _w3) ERC721("MyNFTOriginal", "MNO") {
        w1 = _w1;
        w2 = _w2;
        w3 = _w3;
     }
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (_msgSender() != owner()) {
            if(!_whitelist[_msgSender()]){
                require(isSaleOpen, "Sorry!! can't mint when sale is closed");
            }
            require(msg.value >= assetPrice * _mintAmount, "Must send proper eth value to mint rare tokens");
        }
